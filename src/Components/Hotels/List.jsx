...
const List = () => {
    ...
    const fetchHotel = async ()=>{
      ...
      const mani= result.filter((user)=>{
        const nani= user.city.toLowerCase().includes(destination.trim().toLowerCase())
        return nani;
      }) 
       if (mani.length > 0) {
      setId(mani[0].id);
     ...
     console.log("I am enter here")
          const response2 =   await axios.get(`http://localhost:8080/api/hotel/location?locationId=${id}`)
